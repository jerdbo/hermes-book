...
2. **哪些最紧急？** 3 个 Critical 问题（P-03-01、P-10-01、P-15-01）和 14 个 High 问题构成技术债的核心地雷区，涉及所有四个设计赌注的实现质量。
...
| **High** | 14 | 21.5% | 严重影响设计赌注实现、高频场景性能问题 |
| **Medium** | 34 | 52.3% | 边缘场景不可靠、架构腐化点、可观测性不足 |
| **Low** | 14 | 21.5% | 优化机会、未来隐患、非关键路径问题 |
...
graph TB
    subgraph HeatMap["问题分布热力图 (数字=问题数)"]
        ...
        subgraph Low["Low (14)"]
            L_Arch["Arch: 3<br/>P-03-03, P-13-04<br/>P-15-05"]
...
2. **压力带（橙色）**：High 级 14 个问题分布在四个维度
   - 架构腐化 (4)、性能瓶颈 (4)、可靠性债务 (3)、安全漏洞 (3) 形成全方位压力
...
graph TD
    ...
    subgraph RelLayer["可靠性雪崩层"]
        P_03_03["P-03-03 Low<br/>Grace Call死代码"]
...
1. **炸药包引线（Critical → High）**：
   ...
   - `P-03-01` → `P-10-01 (无沙箱)`
     因果：重构成本高延缓沙箱实现 → Run Anywhere 赌注受阻
...
**目标**：解决 14 个 High 问题，重构四大子系统。

| 章节 | 标题 | 核心任务 | 解决问题 | 设计赌注 |
|-----|------|---------|---------|---------|
| Ch 21 | **显式状态机** | `enum AgentState` + 状态转换图 + 可视化 | P-03-02 (High) | Learning Loop 基础 |
...
- **严重程度**：3 Critical、14 High、34 Medium、14 Low
...
2. **阶段 II（Ch 21-26）**：核心重构，解决 14 High 问题
